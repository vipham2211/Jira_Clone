          ...
          <SelectItem key={item[itemKey]}>
            {IconComponent && (
              <IconComponent
                ...
                size={iconSize}
                top={iconTop}
                avatarUrl={item.avatar ? item.avatar : null}
              />
            )}
            <LabelComponent
              color={itemKey === "statusId" ? taskStatus[item.statusId] : null}
            >
              {item[labelKey]}
            </LabelComponent>
          </SelectItem>
  ...
  return (
    <StyledTaskDetail>
      ...
      <TopActions>
        ...
        />
        <TopActionsRight>
          <Button  onClick={() => window.open('https://github.com/vipham2211/Jira_Clone', '_blank')}  icon="feedback" variant="empty">
            Give feedback
          </Button>
      ...
      <Content>
        ...
        <Right>
          <Status
            renderItem={renderItem}
            renderItemStatusValue={renderItemStatusValue}
            genOptions={genOptions}
            taskDetail={taskDetail}
            updateField={updateField}
          />
